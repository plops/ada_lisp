...
- `Lisp.Eval`: intended fuel and contract proof, current status core proof project clean, including bounded postconditions for positive-fuel literal atoms and structurally well-formed `quote` forms, and now also a shared immediate-form theorem saying any positive-fuel immediate literal-or-quote expression evaluates to `Lisp.Runtime.Immediate_Result` without changing the runtime state
